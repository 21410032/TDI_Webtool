...
def perform_calculations(base_data_df, user, year):
    ...
    year_list = [2022]*len(unique_tribes)
...
# For indicator contribution to dimensions:
    columns_list = [
        'HH_Score_H_CD', 'HH_Score_H_IMM', 'HH_Score_H_MC', 
        'HH_Score_H_U5CM', 'HH_Score_H_FS','H_DS', 
        'HH_Score_E_LE', 'HH_Score_E_DRO','E_DS', 
        'HH_Score_S_IC', 'HH_Score_S_OWN', 'HH_Score_S_SANI', 'HH_Score_S_Fuel', 'HH_Score_S_SoDrWa', 
        'HH_Score_S_ELECTR', 'HH_Score_S_ASS','S_DS', 
        'HH_Score_C_L', 'HH_Score_C_Arts','C_DS', 
        'HH_Score_G_EV', 'HH_Score_G_meeting','G_DS'
    ]


    column_sums_selected = HH_score_df[columns_list].sum()
    total_HS = column_sums_selected['H_DS']
    total_ES = column_sums_selected['E_DS']
    total_SS = column_sums_selected['S_DS']
    total_CS = column_sums_selected['C_DS']
    total_GS = column_sums_selected['G_DS']


    CD_contri_to_H = column_sums_selected['HH_Score_H_CD'] / total_HS if total_HS != 0 else 0
    IM_contri_to_H = column_sums_selected['HH_Score_H_IMM'] / total_HS if total_HS != 0 else 0
    MC_contri_to_H = column_sums_selected['HH_Score_H_MC'] / total_HS if total_HS != 0 else 0
    CM_contri_to_H = column_sums_selected['HH_Score_H_U5CM'] / total_HS if total_HS != 0 else 0
    FS_contri_to_H = column_sums_selected['HH_Score_H_FS'] / total_HS if total_HS != 0 else 0

    LE_contri_to_E = column_sums_selected['HH_Score_E_LE'] / total_ES if total_ES != 0 else 0
    DRO_contri_to_E = column_sums_selected['HH_Score_E_DRO'] / total_ES if total_ES != 0 else 0

    IC_contri_to_S = column_sums_selected['HH_Score_S_IC'] / total_SS if total_SS != 0 else 0
    OW_contri_to_S = column_sums_selected['HH_Score_S_OWN'] / total_SS if total_SS != 0 else 0
    SANI_contri_to_S = column_sums_selected['HH_Score_S_SANI'] / total_SS if total_SS != 0 else 0
    FUEL_contri_to_S = column_sums_selected['HH_Score_S_Fuel'] / total_SS if total_SS != 0 else 0
    DRWA_contri_to_S = column_sums_selected['HH_Score_S_SoDrWa'] / total_SS if total_SS != 0 else 0
    ELECTR_contri_to_S = column_sums_selected['HH_Score_S_ELECTR'] / total_SS if total_SS != 0 else 0
    ASS_contri_to_S = column_sums_selected['HH_Score_S_ASS'] / total_HS if total_HS != 0 else 0

    LAN_contri_to_C = column_sums_selected['HH_Score_C_L'] / total_CS if total_CS != 0 else 0
    ARTS_contri_to_C = column_sums_selected['HH_Score_C_Arts'] / total_CS if total_CS != 0 else 0

    EV_contri_to_G = column_sums_selected['HH_Score_G_EV'] / total_GS if total_GS != 0 else 0
    MEET_contri_to_G = column_sums_selected['HH_Score_G_meeting'] / total_GS if total_GS != 0 else 0
    ...
    Final_Excel['CD_contri_to_H'] = CD_contri_to_H
    Final_Excel['IM_contri_to_H'] = IM_contri_to_H
    Final_Excel['MC_contri_to_H'] = MC_contri_to_H
    Final_Excel['CM_contri_to_H'] = CM_contri_to_H
    Final_Excel['FS_contri_to_H'] = FS_contri_to_H

    Final_Excel['LE_contri_to_E'] = LE_contri_to_E
    Final_Excel['DRO_contri_to_E'] = DRO_contri_to_E

    Final_Excel['IC_contri_to_S'] = IC_contri_to_S
    Final_Excel['OW_contri_to_S'] = OW_contri_to_S
    Final_Excel['SANI_contri_to_S'] = SANI_contri_to_S
    Final_Excel['FUEL_contri_to_S'] = FUEL_contri_to_S
    Final_Excel['DRWA_contri_to_S'] = DRWA_contri_to_S
    Final_Excel['ELECTR_contri_to_S'] = ELECTR_contri_to_S
    Final_Excel['ASS_contri_to_S'] = ASS_contri_to_S

    Final_Excel['LAN_contri_to_C'] = LAN_contri_to_C
    Final_Excel['ARTS_contri_to_C'] = ARTS_contri_to_C

    Final_Excel['EV_contri_to_G'] = EV_contri_to_G
    Final_Excel['MEET_contri_to_G'] = MEET_contri_to_G
